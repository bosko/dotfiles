...
# Function assumes that rbenv, ruby-build and rbenv-vars are installed.
# I usually install it just by cloning git repositories (rbenv to
# .rbenv and ruby-build and rbenv-vars to .rbenv/plugins.
ruby-install() {
  local VERSION="$1"
  if [ -z "$VERSION" ]; then
  ...
  brew update
  brew install readline openssl ctags
  brew upgrade readline openssl ctags
